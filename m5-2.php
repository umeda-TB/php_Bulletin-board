...
<body>  
    ...
    <?php   
        ...
        $password = 'パスワード';
        $pdo = new PDO
        ($dsn, $user, $password, array(PDO::ATTR_ERRMODE => PDO::ERRMODE_WARNING));
        ...
        if(!empty($_POST["name"]) && !empty($_POST["comment"])){
            ...
            if (!empty($_POST["post_pass"])){
                ...
                if (!empty($_POST["edit_tag"])){
                    ...
                    $id = $edit_tag; 
                    $sql = 'UPDATE tbtest2 SET name=:name,comment=:comment,passward=:passward WHERE id=:id';
                ...
                //編集タグがない場合
                    ...
                    //テーブルに投稿内容を追記する
                    $sql = $pdo -> prepare("INSERT INTO tbtest2 (name, comment, passward) 
                    VALUES (:name, :comment, :passward)");
